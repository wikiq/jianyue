> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [medium.com](https://medium.com/@jaxzin/how-i-turned-a-cheap-weather-station-into-a-personal-devops-dashboard-5c8820790fd5#id_token=eyJhbGciOiJSUzI1NiIsImtpZCI6ImQzN2FhNTA0MzgxMjkzN2ZlNDM5NjBjYTNjZjBlMjI4NGI2ZmMzNGQiLCJ0eXAiOiJKV1QifQ..TUayMJU4MwN2YWgeWbBZV4Z-9T2rqGurPexxhCPexVxXYLx8APtJ4wWwht7VsNgFlLPdTVPtZiGtSk1eeMeu3AYlIjqCpI97jygpNgzDJ3LoufVUwgvjZzsdufHeRnlRarEg4gTPM9Ibwd8YPSAFQLnz4euTQoMOxBejTrrCJSDSDkssQ0O0aAZCG-3ezJq06AsIWYeKXPsSPUIxefzs_ZDWWSy1uG9YAfPlAdtQoTkG1_NEGI_Mv3gEoLZdVdK8EsxI-nstpcAGn0hilIqThtcAckgBGCYHJRlhMJkB5y3QiDJQ3PeY_7FAJ6Y88ah3qFbZnoVQjRVqEyEHUW26Cw)

> Home Assistant, Docker, InfluxDB, Grafana and some amateur RF processing combine to create a fascinat......

Let’s start with the visually-pleasing part, which is probably what brought you here. Here is what my dashboard looks like…

![](https://miro.medium.com/max/1250/1*uGGEtqbi9G_gFrfsqEIUJA.png)Overview of the Grafana dashboard for my weather station
...
![](https://miro.medium.com/max/875/1*jf1ayPb9nP-OrY0tMDXY6w.png)The layout of physical and digital components to make the weather dashboard a reality
...
*   **_The weather sensors_** — [WS2032 Weather Station](https://www.google.com/search?q=WS2032+Anemometer+Weather+Station)  
    _$40–85 USD, depending on the site._ I picked this because I found the [rtl_433](https://github.com/merbanan/rtl_433) project and this unit was listed as supported.
...
![](https://miro.medium.com/max/800/1*BzJX0ldpzm-RtlqcEJO9KQ.gif)The WS2032 in action, after being attached to a light post on my patio.
...
![](https://miro.medium.com/max/875/1*DDrRcg3veyzvsB6H7DM75w.png)
...
![](https://miro.medium.com/max/875/1*GyCJbR8__5wT-Fub5OLqfg.png)Customizing the name and icon of the WS2032 entities makes them quicker to add to cards
...
![](https://miro.medium.com/max/875/1*rFR1h6IyTAshIz6onhvgsA.png)An example of the wind rose plugin from spectraphilic.
...
from(bucket: "fallen-leaf")
  ...
  |> filter(fn: (r) => r["entity_id"] == "ws2032_28817_wd" or r["entity_id"] == "ws2032_28817_ws")
  |> filter(fn: (r) => r["_field"] == "value")
  |> aggregateWindow(every: ${period}, fn: mean, createEmpty: false)
  |> group()
  |> pivot(rowKey:["_time"], columnKey: ["entity_id"], valueColumn: "_value") 
  |> filter(fn: (r) => exists r["ws2032_28817_wd"] and exists r["ws2032_28817_ws"]) 
  |> rename(columns: {"ws2032_28817_wd": "direction", "ws2032_28817_ws": "speedKmps"})
  |> map(fn: (r) => ({ r with  speed: r.speedKmps * 0.621371 })) 
  |> keep(columns: ["_time", "speed", "direction"])
  |> aggregateWindow(every: ${period}, fn: first, column: "direction", createEmpty: false)
```
...
![](https://miro.medium.com/max/875/1*JHuBj4zqINtRh02v0PWodw.png)Inspecting the panel shows the raw table of data returned by the Flux query.
...
![](https://miro.medium.com/max/875/1*36e8W_v8XdQNhhpBOHmh4w.png)Wind & Gust Speed on one graph
...
![](https://miro.medium.com/max/875/1*DZ-c5ex7VcbyP6EHw65PLw.png)Wind Speed as a heatmap
...
![](https://miro.medium.com/max/875/1*rXxXLzSNP1V4QPZewp9PWw.png)Charting percentiles to create a flowing river of wind direction
